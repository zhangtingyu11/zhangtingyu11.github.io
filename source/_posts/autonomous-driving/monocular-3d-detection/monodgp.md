...
<strong>Visual Encoder 输出的是图像各位置的视觉特征，不是 2D Query。</strong> Visual Decoder 同时接收两类输入：一组可训练的初始查询，以及视觉编码器输出的特征。查询在交叉注意力中读取视觉特征，更新后的结果才叫 2D Query。

| 名称 | 来源与作用 |
|---|---|
| 视觉特征 | 当前图像经过 Visual Encoder 得到，提供各位置的信息 |
| 初始 Query | 模型学习的一组特征向量，初始时没有指定每个查询负责哪辆车 |
| 2D Query | 初始查询经过 Visual Decoder 更新后的特征，供二维头预测，并继续送入三维解码器 |

因此，图 2 中从 Visual Encoder 指向 Visual Decoder 的箭头，是供查询读取的视觉特征输入；它和从上方进入解码器的查询是两路不同的信息。

### 与 MonoDETR 的结构区别

<strong>三维解码器的 D → I → V 顺序沿用 MonoDETR；新增的是前面的二维视觉解码阶段，以及二维头的分出位置。</strong>

| 对比项 | MonoDETR | MonoDGP |
|---|---|---|
| 查询的处理路径 | 初始查询直接进入深度引导解码器 | 先经过二维视觉解码器，再进入深度引导解码器 |
| 二维头使用的特征 | 深度引导解码后的查询 | 二维视觉解码后的查询 |
| 三维头使用的特征 | 深度引导解码后的查询 | 以 2D Query 为起点，继续深度引导解码后的查询 |

就整个方法而言，另两项变化是 RSH 前景增强与区域向量，以及最终深度由三路平均改为“几何初值＋预测残差”。查询解耦并不包含这两项，它们在表 5 中有独立的开关。

下面只画查询的传递和两个预测头的连接位置，省略了视觉、深度特征输入箭头。“预测头”把查询特征转换为框、尺寸等数值。上半图对应 MonoDETR 的共享查询方式；下半图让二维头提前输出，查询再继续传入三维解码器。
...
<strong>“解耦”指二维头与三维头使用不同阶段的查询，不是去掉三维解码器里的视觉注意力，也不是两套完全独立的网络。</strong>

### 解耦查询的消融证据

论文表 5 的 Decoupled Query 列控制这项结构改动。下面两组都固定其他开关，只比较是否使用解耦查询；指标为 KITTI val Car Moderate 的 $\mathrm{AP}_{3D}$，按作者的五次训练协议报告中位结果。

| 固定的配置 | 不使用解耦查询 | 使用解耦查询 | 差值 |
|---|---:|---:|---:|
| 使用 Mixup3D，不使用 RSH 和深度残差 | 20.79（表 5 第 2 行） | 21.38（第 3 行） | +0.59 个百分点 |
| 使用 Mixup3D、RSH 和深度残差 | 21.48（第 6 行） | 22.34（第 7 行） | +0.86 个百分点 |

原表和模块开关示意保留在后面的“模块收益与计算代价”一节。表 2 还报告了第二组的开销：去掉解耦查询为 39 ms，完整模型为 42 ms，均在 RTX 3090、batch size 1 下测量。

<strong>这些结果支持“增加二维解码器并让二维头提前输出”这一整体设计。</strong> 论文没有在该组实验里进一步拆分“多一个解码器”和“二维头提前分出”的各自贡献，也没有单独证明收益就是来自消除深度对二维定位的干扰；后者是作者的设计动机。
