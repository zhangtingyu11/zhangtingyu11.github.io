...
<div class="distortion-concept-compare">
  <div>
    <small>桶形畸变</small>
    <strong>东西没变，位置变了</strong>
    <p>本来只拍前方这块区域，镜头却把边缘的点推向了错误位置。弯曲是成像误差，通常需要校正。</p>
    <span>同一视场 → 点的位置跑偏</span>
  </div>
  <div>
    <small>鱼眼投影</small>
    <strong>为了装得更多，边缘被压缩</strong>
    <p>它要把左边、右边甚至接近身旁的景物一起塞进照片。内容更多，边缘直线自然会明显弯曲。</p>
    <span>更宽视场 → 更多内容被压进来</span>
  </div>
</div>

回到上面的动画，别只盯着车道线：桶形模式里仍是原来的三栋建筑；切到鱼眼后，左右两侧会多出原本在针孔画面之外的建筑——这才是两者最直观的区别。
