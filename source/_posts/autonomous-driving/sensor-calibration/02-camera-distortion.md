...
这里的“半径”确实是**平面内的距离**：从归一化坐标原点 `O = (0, 0)` 到投影点。平面上两点的距离，用连接它们的直线段长度来量，所以才会画成一条线；它仍然完全位于归一化成像平面内。

`O` 是光轴穿过这个平面的位置。乘上内参后，它对应像素坐标里的主点 `(cx, cy)`，不一定是图片的几何中心。
...
动画中点始终在同一条线上，因为 `(x/r, y/r)` 只保留方向。把前面的步骤连起来，从理想归一化坐标 `(x, y)` 到像素坐标 `(u, v)` 的完整公式是：
...
通常 `s = 0`，于是 `u = fx·xd + cx`。如果 `r = 0`，点就在光轴上，不需要除法，直接得到主点 `(u, v) = (cx, cy)`。
